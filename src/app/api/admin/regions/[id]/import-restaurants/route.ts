import { type NextRequest, NextResponse } from "next/server";

import { parseRestaurantsCsv } from "@/lib/restaurants/csv-import";
import { importRestaurantsCsv } from "@/lib/restaurants/csv-import-engine";
import { requireAdmin } from "@/lib/toolbox/admin";

/**
 * POST /api/admin/regions/[id]/import-restaurants — import a restaurants
 * CSV (admin). body: { cuisine: string, csv: string }
 */

export const maxDuration = 120;

type Ctx = { params: Promise<{ id: string }> };

export async function POST(req: NextRequest, { params }: Ctx) {
  const { id } = await params;
  const { user, isAdmin } = await requireAdmin();
  if (!user) {
    return NextResponse.json(
      { error: "Not signed in — log in with your admin account first." },
      { status: 401 },
    );
  }
  if (!isAdmin) {
    return NextResponse.json(
      { error: "This account is not an admin." },
      { status: 403 },
    );
  }

  const body = (await req.json().catch(() => null)) as {
    cuisine?: string;
    csv?: string;
  } | null;

  const cuisine = body?.cuisine?.trim() || "other";
  if (!body?.csv || body.csv.trim().length === 0) {
    return NextResponse.json({ error: "The CSV file is empty." }, { status: 400 });
  }

  const { rows, errors } = parseRestaurantsCsv(body.csv);
  if (rows.length === 0) {
    return NextResponse.json(
      { error: errors[0] ?? "No valid rows found in the CSV." },
      { status: 400 },
    );
  }

  const result = await importRestaurantsCsv(id, cuisine, rows);
  return NextResponse.json({ ...result, parsed: rows.length, rowErrors: errors });
}
