...
/** Admin Events CSV importer — mirrors the Stays/Experiences importers. */
export function EventsCsvImport({ regionId }: { regionId: string }) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/regions/${regionId}/import-events`, {
        method: "POST",
  ...
  return (
    <div className="rounded-2xl bg-surface p-4 shadow-card ring-1 ring-border">
      <h2 className="text-sm font-bold">Import events from CSV</h2>
      <p className="mt-0.5 text-xs text-muted">
        Upload a CSV — rows match existing events in this region by location.
        Matches refresh details; new events are added.
      </p>

      <div className="mt-3 flex flex-wrap items-end gap-3">
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-muted">Default category</span>
          <select
            value={category}
      ...
      <p className="mt-3 text-[11px] text-muted">
        Expected columns: <code>Title</code>, <code>Category</code>{" "}
        (Morning/Midday/Nighttime), <code>Event Type</code>, <code>When</code>,{" "}
        <code>Description</code>, <code>Rating</code>, <code>Reviews</code>,{" "}
        <code>Phone</code>, <code>WhatsApp</code>, <code>Instagram</code>,{" "}
        <code>Facebook</code>, <code>Address</code>, <code>Website</code>,{" "}
        <code>Image</code>, <code>Amenities</code>, <code>Latitude</code>,{" "}
        <code>Longitude</code>, <code>Google Maps Link</code>. Case-insensitive.
      </p>
    </div>
  );
}
