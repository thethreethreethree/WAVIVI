...
/** Modal editor for a single stay row. Saves via PATCH. */
export function StayEditor({ stay, onClose }: Props) {
  const router = useRouter();
  const [name, setName] = useState(stay.name);
  const [stayType, setStayType] = useState<StayType>(stay.stay_type);
  ...
  const [rating, setRating] = useState(stay.backpack_rating);
  const [googleRating, setGoogleRating] = useState(
    stay.rating != null ? String(stay.rating) : "",
  );
  const [reviewCount, setReviewCount] = useState(String(stay.review_count));
    ...
    >
      ...
      >
        <div className="sticky top-0 flex items-center justify-between border-b border-border bg-surface px-4 py-3">
          <h3 className="text-sm font-bold">Edit stay</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
