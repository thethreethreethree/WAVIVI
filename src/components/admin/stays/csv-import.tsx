...
/**
 * Admin Stays CSV importer — upload a CSV of hostels / hotels / etc. The
 * picked stay type becomes the default for rows whose Industry column is
 * blank; rows that name a different type override it. Imports match by
 * location: nearby existing stays are updated, new rows are inserted.
 */
export function StaysCsvImport({ regionId }: { regionId: string }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [stayType, setStayType] = useState<StayType>("hostel");
  ...
  async function handleImport() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/regions/${regionId}/import-stays`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ stayType, csv }),
        },
      );
  ...
  return (
    <div className="rounded-2xl bg-surface p-4 shadow-card ring-1 ring-border">
      <h2 className="text-sm font-bold">Import stays from CSV</h2>
      <p className="mt-0.5 text-xs text-muted">
        Upload a CSV — rows match existing stays in this region by location.
        Matches refresh rating, reviews, contacts &amp; photo; new stays
        are added.
      </p>

      <div className="mt-3 flex flex-wrap items-end gap-3">
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-muted">Stay type</span>
          <select
            value={stayType}
